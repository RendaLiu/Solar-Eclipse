...
def main():
    # 设置参数
    YEARS = 10          # 模拟10年
    COARSE_DT = 1   # 粗略时间步长6小时
    FINE_DT = 0.1         # 精细时间步长0.1小时
    START_YEAR = 2025
    start_year = Time(START_YEAR, format='jyear') + 6 * u.hour # 从2025年开始
    end_year = Time(START_YEAR, format='jyear') + 6 * u.hour + YEARS * u.year
    
    print(f"开始计算{start_year.iso}至{end_year.iso}的日月食预报...")
    ...
    print(f"使用参数：粗略时间步长 = {COARSE_DT:.1f}小时，精细时间步长 = {FINE_DT * 60}分钟\n")
    ...
    if not eclipse_events['solar']:
        ...
        for i, event in enumerate(eclipse_events['solar'], 1):
            start_time = (start_year + event['start_time'] * u.hour).iso
            max_time = (start_year + event['max_time'] * u.hour).iso
            end_time = (start_year + event['end_time'] * u.hour).iso
